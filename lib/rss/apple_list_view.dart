...
class _AppleListView extends State<AppleListView>{
  ...
  getHttpData() async {
    ...
    var url = "https://rss.itunes.apple.com/api/v1/cn/podcasts/top-podcasts/all/100/explicit.json";//播客
...
class _AppleItemList extends State<AppleItemList>{
  ...
  Widget build(BuildContext context) {
    ...
    return new ListTile(
      title: Text(widget._itemBean.artistname==null?"":widget._itemBean.name),
      subtitle: Text(widget._itemBean.artistname==null?"":widget._itemBean.artistname),
      leading: new CircleAvatar(child: Image.network(
